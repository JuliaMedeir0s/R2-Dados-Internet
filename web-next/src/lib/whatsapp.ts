...
export const useUtm = () => {
  ...
  useEffect(() => {
    ...
    // `window`/`document` só existem no cliente, então esse valor não pode
    // ser calculado durante o render (que também roda no servidor) — por
    // isso o setState acontece aqui, uma única vez após o mount, em vez de
    // ser derivado no corpo do componente.
